...
-- Partial Indexes para Unicidad de Usuarios (Solo si no está borrado)
CREATE UNIQUE INDEX idx_users_email_active ON paas_core.users(email) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX idx_users_code_active ON paas_core.users(code) WHERE deleted_at IS NULL;
...
CREATE TABLE paas_core.projects (
    ...
    name VARCHAR(255) NOT NULL,
    description TEXT,
    -- NOTA: Se elimina UNIQUE de aquí para manejarlo con un índice parcial.
...
-- Partial Index para Unicidad de Proyectos:
-- 1. Un usuario no puede tener dos proyectos activos con el mismo nombre
CREATE UNIQUE INDEX idx_projects_name_user ON paas_core.projects(user_id, name) WHERE deleted_at IS NULL;
-- 2. El namespace en Kubernetes debe ser globalmente único para proyectos activos
CREATE UNIQUE INDEX idx_projects_namespace_active ON paas_core.projects(namespace_name) WHERE deleted_at IS NULL;
