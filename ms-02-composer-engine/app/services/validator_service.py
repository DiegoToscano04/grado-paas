...
class ComposeValidatorService:
    ...
    def validate(
        self, compose_content: str, architecture: AppArchitecture
    ) -> Tuple[bool, List[str]]:
        errors = []
        ...
        try:
            ...
            if not isinstance(compose_dict, dict):
                return False, [
                    "El archivo YAML no tiene un formato válido de diccionario en la raíz."
                ]
        except yaml.YAMLError as e:
            return False, [f"Error de sintaxis YAML: {str(e)}"]

        # 1.5 Validación de Atributos Raíz (Indentación estricta)
        ALLOWED_ROOT_KEYS = {"version", "services"}
        actual_root_keys = set(compose_dict.keys())
        forbidden_root_keys = actual_root_keys - ALLOWED_ROOT_KEYS

        if forbidden_root_keys:
            return False, [
                f"Error de indentación o atributo no permitido en la raíz del documento: {', '.join(forbidden_root_keys)}. Asegúrate de indentar correctamente con espacios."
            ]
        # -------------------------------
        ...
        if expected_services != actual_services:
            ...
            if missing:
                errors.append(
                    f"Faltan los siguientes servicios requeridos para {architecture.name}: {', '.join(missing)}"
                )
            if extra:
                errors.append(
                    f"Servicios no permitidos encontrados: {', '.join(extra)}. Solo se permiten: {', '.join(expected_services)}"
                )
        ...
        for service_name, service_config in services.items():
            if not isinstance(service_config, dict):
                errors.append(
                    f"La configuración del servicio '{service_name}' no es válida."
                )
                continue
            ...
            if forbidden_keys:
                errors.append(
                    f"El servicio '{service_name}' contiene configuraciones no permitidas: {', '.join(forbidden_keys)}."
                )

            if "build" in actual_keys:
                errors.append(
                    f"No se permite construir imágenes ('build' en '{service_name}'). Usa imágenes de DockerHub."
                )
            ...
            if is_web_component and "ports" not in actual_keys:
                errors.append(
                    f"El servicio web '{service_name}' debe exponer al menos un puerto ('ports')."
                )
